...
public class GameBoxUtil {
	...
	public static void pay(final Activity activity, final String price, final String productName,
			final String productDesc, final String requestId, final GameEventHandler handler) {
		...
		// company name of CP.
		payInfo.put(GlobalParam.PayParams.USER_NAME, "XXX公司");
		// 必填字段，不能为null或者""
		// the APP ID is required and can not be null or "".
	...
	public static void startPay(final Activity activity, final String price, final String productName,
			final String productDesc, final String requestId, final GameEventHandler handler) {
		if ("".equals(GlobalParam.PAY_RSA_PRIVATE)) {
			ReqTask getPayPrivate = new ReqTask(new Delegate() {
				...
				public void execute(String privateKey) {
					/**
					 * 从服务端获取的支付私钥，由于没有部署最终的服务端，所以返回值写死一个值，CP需要从服务端获取，
					 * 服务端代码参考华为Demo 请查阅 华为游戏中心SDK开发指导书.docx 的2.5节
					 */
					/**
					 * The demo app did not deployed the server, so the return
					 * value is written fixed.For real app,the CP need to get
					 * the key from server.
					 */
					privateKey = "";
