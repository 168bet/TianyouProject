...
public class Helpers {
    ...
    static public boolean doesFileExist(Context c, String fileName, long fileSize,
        ...
        return doesFileExist(c, fileName, fileSize, null, deleteFileOnMismatch);
    }
     ...
     * @param fileName             the name (sans path) of the file to query
     * @param fileSize             the size that the file must match
     * @param fileMD5              the md5 that the file must match
     * @param deleteFileOnMismatch if the file sizes do not match, delete the
     *                             file
     * @return true if it does exist, false otherwise
     */
    static public boolean doesFileExist(Context c, String fileName, long fileSize, String fileMD5,
                                        boolean deleteFileOnMismatch) {
        // the file may have been delivered by Market --- let's make sure
        // it's the size we expect
        String inputFile = Helpers.generateSaveFileName(c, fileName);
        File fileForNewFile = new File(inputFile);
        try {
            ...
            if (fileForNewFile.exists()) {
                if (fileForNewFile.length() == fileSize) {
                    if (fileMD5 == null) {
                        return true;
                    } else if (fileMD5.equals(fileMD5(fileMD5))) {
                        return true;
                    }
                }
