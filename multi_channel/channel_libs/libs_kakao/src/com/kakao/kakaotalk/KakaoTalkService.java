...
public class KakaoTalkService {
     ...
     * @param args 메시지 템플릿에 정의한 arg key:value. 템플릿에 정의된 모든 arg가 포함되어야 함.
     */
    public static void requestSendMemo(final TalkResponseCallback<Boolean> callback,
                                          final String templateId,
                                          final Map<String, String> args) {
        KakaoTaskQueue.getInstance().addTask(new KakaoResultTask<Boolean>(callback) {
            @Override
            public Boolean call() throws Exception {
                return KakaoTalkApi.requestSendMemo(templateId, args);
            }
        });
    }
    ...
    public static void requestChatRoomList(final TalkResponseCallback<ChatListResponse> callback, final ChatListContext context) {
        KakaoTaskQueue.getInstance().addTask(new KakaoResultTask<ChatListResponse>(callback) {
            @Override
            public ChatListResponse call() throws Exception {
                return KakaoTalkApi.requestChatRoomList(context);
            }
        });
    }
}
