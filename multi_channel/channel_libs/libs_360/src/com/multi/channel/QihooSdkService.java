...
import com.qihoo.payment.QihooPayInfo;
...
public class QihooSdkService extends BaseSdkService {
	 ...
	 * 角色信息采集接口
	 */
	protected void doSdkGetUserInfoByCP(String type,RoleInfo roleinfo) {
		HashMap eventParams = new HashMap();
		eventParams.put("type", type);// 角色信息接口触发的场景
		eventParams.put("zoneid", roleinfo.getServerId());// 当前角色所在游戏区服id
		eventParams.put("zonename", roleinfo.getServerName());// 当前角色所在游戏区服名称
		eventParams.put("roleid",roleinfo.getRoleId() );// 当前角色id
		eventParams.put("rolename", roleinfo.getRoleName());// 当前角色名称
		eventParams.put("professionid", 0);//登录玩家的职业ID
		eventParams.put("profession", roleinfo.getProfession());//当前登录玩家的职业名称
		eventParams.put("gender", "无");//当前登录玩家的性别
		eventParams.put("rolelevel", roleinfo.getRoleLevel());//当前登录的玩家角色等级
		eventParams.put("power", 0);//战力值
		eventParams.put("vip", roleinfo.getVipLevel());//当前用户VIP等级
		JsonObject jsonObject = new JsonObject();
		jsonObject.addProperty("balanceid", 0);
		jsonObject.addProperty("balancename", "无");
		jsonObject.addProperty("balancenum", roleinfo.getBalance());
		eventParams.put("balance", jsonObject);
		eventParams.put("partyid", 0);//当前用户所属帮派帮派ID
		eventParams.put("partyname", roleinfo.getParty());//当前用户所属帮派名称
		eventParams.put("partyroleid", 0);//帮派称号ID
		eventParams.put("partyrolename", "无");//帮派称号名称
//		JsonObject js = new JsonObject();
		eventParams.put("friendlist", "无");//关系角色ID(roleid)
