...
public class VivoSdkService extends BaseSdkService{
	...
	private MyAccoutChangeListener mAccountListener;
	private MyPayReslutListener mPayReslutListener;
	...
	public void doActivityInit(Activity activity,
		...
		mUnionManager = new VivoUnionManager(mActivity);
		// 切换账号的监听
		mAccountListener = new MyAccoutChangeListener();
		mUnionManager.registVivoAccountChangeListener(mAccountListener);
		
		mUnionManager.bindUnionService();
	...
	public void doDestory() {
		...
		if (mPayReslutListener != null) { mUnionManager.cancelVivoPaymentAndRecharge(mPayReslutListener); }
	...
	private class MyAccoutChangeListener implements OnVivoAccountChangedListener {
		...
		public void onAccountLogin(String name, String openid, String authtoken) {
			...
			mActivity.runOnUiThread(new Runnable() {
				@Override
				public void run() {	mUnionManager.showVivoAssitView(mActivity);	}	// 登录成功后显示悬浮框
			});
		}

		@Override
		public void onAccountLoginCancled() { mTianyouCallback.onResult(TianyouCallback.CODE_LOGIN_CANCEL, "取消登录"); }

		@Override
		public void onAccountRemove(boolean isRemoved) { Log.d("TAG", "account remove-------------"); }
	}
	...
	private class MyPayReslutListener implements OnVivoPayResultListener{
		...
		public MyPayReslutListener (String orderID) { this.orderID = orderID; }
		
		@Override
		public void payResult(String transNo, boolean pay_result, String result_code,String pay_msg) {
			
			String msg = "payResult: transNo= "+transNo+",pay_result= "+pay_result+",result_code= "+result_code+",pay_msg= "+pay_msg;
			if ("9000".equals(result_code)) { checkOrder(orderID); } 
			
			else if ("6001".equals(result_code)) { mTianyouCallback.onResult(TianyouCallback.CODE_PAY_CANCEL, "支付取消"); } 
			
			else { mTianyouCallback.onResult(TianyouCallback.CODE_PAY_FAILED, "支付失败"); }
		}
		 ...
		 */
		@Override
		public void rechargeResult(String openid, boolean pay_result, String result_code,
				String pay_msg) {
			Log.d("TAG","rechargeResult: openid= "+openid+",pay_result= "+pay_result+",result_code= "+result_code
					+",pay_msg= "+pay_msg);
		}
	}
