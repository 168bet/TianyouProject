$(function(){
    ...
    $("button[type='button']").click(function(){
        ...
        var data=serializeForm($('#login')); //序列化表单元素
        /**
         * 有兴趣的可以到这里 自行发送Ajax请求 实现登录功能
         */
			...
			cache: false,
	   		success: function(data){
				...
				if(data.message == 'success'){
					...
					$("input[name='password']").nextAll('span').eq(0).css({display:'block'}).text(data.message);
				}else{
					$("input[name='userName']").nextAll('span').eq(0).css({display:'block'}).text(data.message);
				}
	   		}
		});
    });
